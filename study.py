...
def set_favicon(image_path):
    ...
    st.markdown(
        f'<head><link rel="shortcut icon" href="data:image/{ext};base64,{img}"></head>',
...
@st.cache_data(ttl=2)
def _load_db_cached():
    """Cache curto para reduzir leituras repetidas no mesmo rerun."""
...
def save_db(db):
    _write_db(db)
    # Limpa cache para que a próxima leitura reflita os dados salvos
...
def get_user_data(username):
    db = load_db()
    ud = db["user_data"].get(username)
    if ud is None:
        ud = {"historico": [], "favoritos": [], "api_key": "", "respostas_provas": {}}
...
def render_avatar(nome, username, size=48):
    ...
    st.markdown(
        ...
        f'font-size:{size//3}px;background:{cor}">{iniciais}</div>',
        unsafe_allow_html=True,
    )
...
def render_tags(q):
    ...
    if tags:
        pills = "".join(
            [f'<span class="tag-pill">{t.strip()}</span>' for t in tags.split(",") if t.strip()]
        )
...
def mostrar_animacao(acertou):
    if acertou:
        ...
        for i in range(25):
            ...
            pieces += (
                f'<div class="confetti-piece" style="left:{x}px;background:{cor};'
...
def q_id(q):
    """ID único da questão — usa 24 chars para reduzir chance de colisão."""
    return _hash(q["enunciado"])[:24]

# ── Session state ──────────────────────────────────────────────────────────
DEFAULTS = {
    "usuario": None,
    "quiz_qs": [], "quiz_idx": 0, "quiz_acertos": 0, "quiz_erros": 0,
    "quiz_respondida": False, "quiz_escolha": None, "quiz_ativo": False,
    "quiz_finalizado": False, "quiz_mat": None, "quiz_dif": None,
    "quiz_tempo_inicio": None, "quiz_tempo_limit": None,
    "quiz_respostas": [], "qs_geradas": [], "editando_q": None,
    "revisao_ativa": False,
    "prova_ativa": None, "prova_idx": 0, "prova_respondida": False,
    "prova_escolha": None, "prova_respostas": [], "prova_finalizada": False,
...
def registrar(nome, username, senha, role="prof"):
    ...
    if len(u) < 3:
        return False, "Usuário muito curto (mín. 3)."
    if len(senha) < 4:
...
def alterar_senha(username, senha_atual, senha_nova):
    ...
    if len(senha_nova) < 4:
        return False, "Nova senha muito curta (mín. 4)."
...
def gerar_questoes_ia(texto, quantidade, materia, dificuldade, api_key, tags=""):
    ...
    prompt = (
        f'Você é um professor criando questões de múltipla escolha de nível {dificuldade} para um cursinho.\n'
        f'Crie exatamente {quantidade} questões sobre "{materia}"{tag_info} a partir do texto abaixo.\n'
        f'REGRAS:\n'
        f'- 4 alternativas (A, B, C, D), apenas uma correta\n'
        f'- Nível {dificuldade}: {"objetivas" if dificuldade=="Fácil" else "interpretação" if dificuldade=="Médio" else "análise aprofundada"}\n'
        f'- Inclua explicação breve do gabarito\n'
        f'- Inclua um campo "tags" com 1-3 subtópicos relevantes separados por vírgula\n'
        f'Responda SOMENTE com JSON válido:\n'
        f'[{{"enunciado":"...","alternativa_a":"...","alternativa_b":"...","alternativa_c":"...","alternativa_d":"...",'
        f'"gabarito":"a","explicacao":"...","dificuldade":"{dificuldade}","tags":"subtópico1, subtópico2"}}]\n'
        f'TEXTO: {texto[:12000]}'
    )
            ...
            with col2:
                ...
                if c["username"] == u["username"]:
                    if st.button("🗑️", key=f"delcom_{qid}_{c['id']}"):
                        db2 = load_db()
                        db2["comentarios"][qid] = [
                            x for x in db2["comentarios"].get(qid, []) if x["id"] != c["id"]
                        ]
                        save_db(db2)
                        st.rerun()
...
def sidebar():
    ...
    with st.sidebar:
        ...
        with col1:
            render_avatar(u["nome"], u["username"], 40)
        with col2:
        ...
        st.divider()
        paginas = [
            "📖 Estudar", "📝 Provas", "🔁 Modo revisão",
            "⭐ Favoritos", "📊 Meu desempenho", "🏆 Ranking", "🔒 Minha conta",
        ]
        if u["role"] == "prof":
            paginas += [
                "➕ Adicionar questão", "🤖 Gerar do PDF",
                "📋 Banco de questões", "👥 Turmas",
                "📋 Criar prova", "📊 Relatório de turma",
            ]
        elif u["role"] == "admin":
...
# ── ESTUDAR ────────────────────────────────────────────────────────────────
def _resetar_quiz(pool, mat_sel, dif_sel, tempo_limit):
    """Inicializa o estado do quiz com o pool de questões dado."""
...
def _salvar_sessao_historico(u, ac, er, pct):
    """Salva resultado da sessão no histórico do usuário (exceto admin)."""
    if u["role"] == "admin":
        return
    ...
    udata["historico"].append({
        ...
        "pct": pct,
        "respostas": [
            {"enunciado": r["q"]["enunciado"], "acertou": r["escolha"] == r["q"]["gabarito"]}
            for r in st.session_state.quiz_respostas
        ],
    })
    save_user_data(u["username"], udata)

def _render_temporizador(tempo_limit):
    """
    Renderiza a barra de tempo e retorna True se o tempo esgotou.
    NÃO usa time.sleep — controla via rerun agendado.
    """
    if not tempo_limit or st.session_state.quiz_respondida:
        return False
    elapsed = time.time() - st.session_state.quiz_tempo_inicio
    restante = max(0.0, tempo_limit - elapsed)
...
def pg_estudar():
    ...
    if not st.session_state.quiz_ativo and not st.session_state.quiz_finalizado:
        with st.container(border=True):
            ...
            todas_tags = sorted(
                set(t.strip() for q in todas for t in q.get("tags", "").split(",") if t.strip())
            )
            ...
            if st.button("▶ Iniciar sessão", type="primary", use_container_width=True):
                pool = todas[:]
                if not mat_sel.startswith("🔀"):
                    pool = [q for q in pool if q["materia"] == mat_sel]
                if not dif_sel.startswith("🔀"):
                    pool = [q for q in pool if q.get("dificuldade") == dif_sel]
                if tag_sel:
                    pool = [q for q in pool if any(t in q.get("tags", "") for t in tag_sel)]
                if not pool:
                    st.error("Nenhuma questão com esses filtros.")
                    return
                if shuffle:
                    random.shuffle(pool)
                pool = pool[:int(qtd)]
                _resetar_quiz(pool, mat_sel, dif_sel, tempo_limit)
                st.rerun()
        return

    # ── Tela de resultado ─────────────────────────────────────────────────
    if st.session_state.quiz_finalizado and not st.session_state.revisao_ativa:
        ...
        if pct >= 70:
            st.success("🎉 Ótimo desempenho!")
        elif pct >= 50:
            st.warning("📚 Resultado razoável.")
        else:
            st.error("💪 Não desista!")

        # Salva apenas uma vez ao chegar nessa tela
        if not st.session_state.get("_sessao_salva"):
            _salvar_sessao_historico(u, ac, er, pct)
            st.session_state["_sessao_salva"] = True
        ...
        if st.button("↺ Nova sessão", type="primary"):
            ...
            st.session_state.quiz_finalizado = False
            st.session_state["_sessao_salva"] = False
            st.rerun()
        return
    ...
    # Verifica tempo — sem time.sleep
    tempo_esgotado = _render_temporizador(tempo_limit)
    if tempo_esgotado and not st.session_state.quiz_respondida:
        ...
        st.session_state.ultima_resposta = "wrong"
        st.rerun()

    st.progress(
        idx / len(qs),
        text=f"Questão {idx+1} de {len(qs)}  —  ✅ {st.session_state.quiz_acertos}  ❌ {st.session_state.quiz_erros}",
    )
    ...
    with st.container(border=True):
        ...
        with col3:
            if u["role"] != "admin":
                ...
                if st.button("⭐" if ja_fav else "☆", key=f"fav_{idx}"):
                    if ja_fav:
                        udata["favoritos"].remove(qid)
                    else:
                        udata["favoritos"].append(qid)
                    save_user_data(u["username"], udata)
                    st.rerun()
        ...
        if not st.session_state.quiz_respondida:
            ...
            if acertou:
                ...
                st.error(f"**Errado!** Resposta: **{q['gabarito'].upper()}**")
    ...
    # Rerun automático para atualizar o temporizador sem time.sleep
...
def pg_revisao():
    ...
    # Coleta questões erradas únicas
    vistas = set()
    erradas_unicas = []
    for h in hist:
        for r in h.get("respostas", []):
            if not r.get("acertou", True):
                ...
                q_orig = todas_por_enunc.get(enunc)
                if q_orig:
                    qid = q_id(q_orig)
                    if qid not in vistas:
                        vistas.add(qid)
                        erradas_unicas.append(q_orig)

    if not erradas_unicas:
        st.info("Nenhuma questão errada encontrada. Complete sessões de estudo primeiro!")
        return
    ...
    if not st.session_state.quiz_ativo and not st.session_state.revisao_ativa and not st.session_state.quiz_finalizado:
        ...
        if st.button("🔁 Iniciar revisão", type="primary", use_container_width=True):
            ...
            if shuffle:
                random.shuffle(pool)
            pool = pool[:int(qtd)]
            _resetar_quiz(pool, "Revisão", "—", None)
            st.session_state.revisao_ativa = True
            st.rerun()
        return

    # ── Resultado da revisão ──────────────────────────────────────────────
    if st.session_state.quiz_finalizado and st.session_state.revisao_ativa:
        ac = st.session_state.quiz_acertos
        er = st.session_state.quiz_erros
        tot = ac + er
        pct = round(ac / tot * 100) if tot else 0
        ...
        if pct >= 70:
            st.success("🎉 Você dominou essas questões!")
        elif pct >= 50:
            st.warning("📚 Melhorando! Continue.")
        else:
            st.error("💪 Continue praticando.")

        # Salva no histórico apenas uma vez
        if not st.session_state.get("_revisao_salva"):
            _salvar_sessao_historico(u, ac, er, pct)
            st.session_state["_revisao_salva"] = True

        if st.button("↺ Revisar novamente", type="primary"):
            st.session_state.quiz_ativo = False
            st.session_state.quiz_finalizado = False
            st.session_state.revisao_ativa = False
            st.session_state["_revisao_salva"] = False
            st.rerun()
        return

    # ── Questão de revisão ────────────────────────────────────────────────
    if st.session_state.quiz_ativo and st.session_state.revisao_ativa:
        qs = st.session_state.quiz_qs
        idx = st.session_state.quiz_idx
        ...
        with st.container(border=True):
            ...
            with col2:
                ...
                if st.button("⭐" if ja_fav else "☆", key=f"rev_fav_{idx}"):
                    if ja_fav:
                        udata3["favoritos"].remove(qid)
                    else:
                        udata3["favoritos"].append(qid)
...
def pg_desempenho():
    ...
    hist = udata.get("historico", [])
    if not hist:
        st.info("Nenhuma sessão concluída ainda.")
        return
        ...
        for i, (mat, d) in enumerate(sorted(por_mat.items())):
            ...
            <div style="margin-bottom:12px">
              ...
              <div style="height:10px;background:rgba(255,255,255,0.08);border-radius:5px;overflow:hidden">
                <div style="height:100%;width:{pct}%;background:{cor};border-radius:5px;transition:width 1s ease"></div>
              </div>
            </div>"""
        st.markdown(
            f'<div style="padding:16px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:12px">{bars_html}</div>',
            unsafe_allow_html=True,
        )
    ...
    for h in hist:
        ...
        if dif not in por_dif:
            por_dif[dif] = {"total": 0, "acertos": 0}
    ...
    for h in reversed(hist):
        ...
        st.caption(
            f"{cor} **{h['data']}** — {h['materia']} | {h.get('dificuldade', '—')} | {h['acertos']}/{h['total']} ({h['pct']}%)"
        )
...
def main():
    ...
    if pg == "📖 Estudar":
        pg_estudar()
    elif pg == "🔁 Modo revisão":
        pg_revisao()
    elif pg == "📊 Meu desempenho":
        pg_desempenho()
    else:
        st.info(f"Página **{pg}** em construção.")
